...
context_prompt=(
        ...
        "{context_str}"
        "\nInstruction: Use only the previous chat history above and context, to interact and help the user. Never give any kinds of links, email addresses or contact numbers in the answer."
        )
...
def get_response(llm,prompt,message_history):
    ...
    if 0 == 0: #feedback.text
        ...
        df = pd.read_csv(f'logs/conversation_logs_saiteja.csv')
        new_row = {'Question': str(prompt), 'Answer': response,'Time':end -start,'Unigram_Recall' : scores[0]["rouge-1"]["r"],'Unigram_Precision' : scores[0]["rouge-1"]["p"],'Bigram_Recall' : scores[0]["rouge-2"]["r"],'Bigram_Precision' : scores[0]["rouge-2"]["r"]}
        df = pd.concat([df, pd.DataFrame(new_row, index=[0])], ignore_index=True)
        df.to_csv(f'logs/conversation_logs_saiteja.csv', index=False)
